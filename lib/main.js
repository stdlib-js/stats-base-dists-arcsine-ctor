...
function Arcsine() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( a ) || isnan( a ) ) {
			throw new TypeError( format( '0u18a', a ) );
		}
		if ( !isNumber( b ) || isnan( b ) ) {
			throw new TypeError( format( '0u18b', b ) );
		}
		if ( a >= b ) {
			throw new RangeError( format( '0u1DV', a, b ) );
		}
	} else {
		a = 0.0;
		b = 1.0;
	...
	defineProperty( this, 'a', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '0u18d', value ) );
			}
			if ( value >= b ) {
				throw new RangeError( format( '0u18e', b, value ) );
			}
			a = value;
		}
	});
	defineProperty( this, 'b', {
		...
		'set': function set( value ) {
			...
			if ( value <= a ) {
				throw new RangeError( format( '0u18f', a, value ) );
			}
			b = value;
		}
	});
